...
if (signupForm) {
    ...
    signupForm.addEventListener(
        ...
        async function (event) {
            ...
            // Only the same username, email, or phone is considered a duplicate.
            const accounts = getAccounts();

            const duplicateAccount = accounts.find(account =>
                String(account.username || "").trim().toLowerCase() === username.toLowerCase() ||
                String(account.email || "").trim().toLowerCase() === email.toLowerCase() ||
                String(account.phone || "").trim() === phone
            );

            if (duplicateAccount) {

                let duplicateMessage = "An account with these details already exists.";

                if (String(duplicateAccount.username || "").trim().toLowerCase() === username.toLowerCase()) {
                    duplicateMessage = "This username already exists. Please choose another username.";
                } else if (String(duplicateAccount.email || "").trim().toLowerCase() === email.toLowerCase()) {
                    duplicateMessage = "This email already exists. Please use another email.";
                } else if (String(duplicateAccount.phone || "").trim() === phone) {
            ...
            const user = {
                ...
                // Accounts created from the Admin "Create Account" page
                // are administrator accounts. Member accounts are created
                // from Members -> Create Login and are linked to a memberId.
                role:
                    "admin"
            ...
            try {
                ...
                // Keep the newly created account as the active account for
